...
- **Màn con `02` "Giao diện" (Sáng/Tối/Theo hệ thống) — đã triển khai (PBI 18)** — hàng "Giao diện" màn `01` đã kích hoạt (bỏ no-op); **dark mode áp toàn app ngay, không restart**, nhớ qua restart bằng row `themeMode` trong `AppSettings` v5 (không migration). Chi tiết màn + cơ chế tại [[Hồ sơ & Bảo mật]], bảng token tại [[Design system]]. Còn lại (PBI sau): màn con `03`–`07` (ngôn ngữ, định dạng & tiền tệ, máy tính, tìm kiếm toàn cục, quản lý tag), **hiệu ứng 2 công tắc** (che số dư thật, đổi bàn phím nhập tiền), theme màu tùy biến, tablet.
...
7. **Nhóm Tiện ích & Cá nhân hóa (§12)**: màn danh sách `01` (gom điểm vào) **đã MVP (PBI 17)**; màn con `02` **Light/Dark mode đã xong (PBI 18)** — quyết định mặc định khi chưa từng chọn là **"Theo hệ thống"** (khớp "System" của Android 13+). Còn **màn con `03`–`07`** (đa ngôn ngữ, định dạng ngày/tiền/kỳ tài chính, máy tính, tìm kiếm, tag) và **hiệu ứng 2 công tắc** — chưa gắn GĐ. Riêng đa ngôn ngữ: cơ chế chốt **GetX Translations** ([[Stack kỹ thuật]]), cần chốt luôn danh sách ngôn ngữ hỗ trợ.
