...
|---|---|---|
| **MVP** | Khóa app (PIN/sinh trắc), Quản lý ví, Thêm/sửa/xóa giao dịch, Danh mục, Báo cáo cơ bản (pie/bar) — **màn `01` Tổng quan (PBI 22) + màn `02` Chi tiết theo danh mục (PBI 23) đã xong** | — |
...
- **Báo cáo (MVP) — màn `01` Tổng quan (PBI 22) + màn `02` Chi tiết theo danh mục (PBI 23) đã triển khai** — màn `01` (màn cấp tab, có bottom nav): segmented control **4 kỳ** (Ngày/Tuần/Tháng/Năm, mặc định Tháng), 2 số tổng, **biểu đồ dòng tiền 6 đơn vị** kết thúc ở kỳ đang chọn, **vòng tròn phân bổ** top 5 danh mục cha + "Khác", **top danh mục chi tiêu** (≤5, có thanh tiến độ), **trạng thái rỗng** theo kỳ, **drill-down** chạm danh mục → tab Giao dịch đã lọc sẵn, và liên kết **"Xem tất cả"** trên thẻ top. Màn `02` (**màn con đè shell**, không bottom nav): chip kỳ **nhãn tĩnh**, vòng tròn + danh sách **đầy đủ** mọi danh mục (không cắt top 5) với màu định tính **theo hạng, lặp chu kỳ**, drill-down như màn `01` nhưng `popUntil` trước khi đổi tab, trạng thái rỗng **toàn màn**. **Không schema riêng, không cache** — chỉ đọc `transactions` + `categories` (schema **v7**), màn `02` dựng từ **bản chụp RAM**. Hàng "Ngân sách" giữ **trong** tab Báo cáo, đặt trước các thẻ (chốt 2026-09-12). Chi tiết tại [[Báo cáo]].
  - Còn lại của module Báo cáo: 2 màn **`03` So sánh Kỳ**, **`04` Xuất báo cáo (PDF/Excel/CSV)** + **bộ lọc báo cáo nâng cao** (khoảng ngày tuỳ chỉnh — tức **nút biểu tượng lịch** của mockup `01`, hiện **không** hiển thị; lọc theo ví/danh mục/tag; ghi nhớ bộ lọc), biểu đồ **xu hướng (line)** + đường trung bình động, báo cáo dòng tiền theo từng ví, insight tự động, toggle "xem theo danh mục con", kéo/vuốt quá 6 đơn vị.
