...
## Thông báo hệ điều hành — kênh, icon, dòng trạng thái quyền (PBI 31)
- **Coral vẫn CHỈ ở cảnh báo chi tiêu** (FR-006/SC-001): trong **3 kênh Android**, `sora_budget` là kênh duy nhất mang sắc coral (`AndroidNotificationDetails.color = AppColors.coral`); `sora_daily` + `sora_summary` teal. **Màu token, không hex cứng** — rà soát cuối khẳng định `lib/core/notification/*` **0** hex ngoài `AppColors`/`SoraColors`.
- **Icon nhỏ của Android buộc là drawable ĐƠN SẮC** (`ic_notif_bell.xml` / `ic_notif_warning.xml` / `ic_notif_summary.xml`, 24dp, `tint` trắng, một `path` duy nhất) — **không** dùng `Icons.*` (font icon) và **không** dùng ảnh launcher: hệ điều hành chỉ đọc **kênh alpha** rồi tự tô màu, ảnh nhiều màu sẽ ra một khối trắng. Bộ hình bám icon màn Trung tâm (chuông / tam giác cảnh báo / biểu đồ tròn).
- **Dòng trạng thái quyền ở màn `01`** (FR-032): `Container` nền `softCardBg` bo `10`, `margin (20, 16, 20, 0)` — icon `notifications_off_outlined` `20` `textSecondary` + câu `12` `textSecondary` + `TextButton('Mở cài đặt')`; **đúng một** dòng, đặt **trên** nhãn nhóm đầu tiên; quyền đã cấp ⇒ **0** dòng. **Cố ý KHÔNG dùng coral** — đây không phải ngữ cảnh chi tiêu/cảnh báo số liệu (DS dành coral cho chi/cảnh báo).
- **Hộp thoại soft-ask** dùng `AlertDialog` Material mặc định (2 `TextButton`: "Không đồng ý" / "Đồng ý") — **không** thêm theme/dialog riêng; tên app "Sora Thu Chi" trong lời giải thích **không dịch** (N4).
